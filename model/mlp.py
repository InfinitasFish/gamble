...
from preproc.xy import split_sequence, split_seq_xy_pipe, NormType, normalize_seq_uni
...
def fit_mlp_uni_reg(mlp_reg: MLPRegressor, ts_seq: np.ndarray, seq_len: int, norm_type: NormType=NormType.NoNorm,
    ...
    ts_seq = normalize_seq_uni(ts_seq, norm_type)
    X, y = split_sequence(ts_seq, seq_len)
    mlp_reg.fit(X, y)
    return mlp_reg


# TODO: we have to pass a Scaler object for particular fitted model, also denormalize final prediction
def predict_uni_next_price(mlp_reg: MLPRegressor, ts_seq: np.ndarray, seq_len: int, norm_type: NormType=NormType.NoNorm)\
        -> float:
