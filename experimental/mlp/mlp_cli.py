...
def main():
    ...
    mlp_params_distr = {"hidden_layer_sizes": [(100, 100), (150, 150), (300, 300), (100, 100, 100)],
                        "learning_rate_init": stats.uniform(0.001, 0.1),
                        "alpha": stats.uniform(0.0001, 0.1),
                        "shuffle": [False],
                        "max_iter": stats.randint(2000, 6000),}

    # getting data
    X_candles, y_candles = get_candles_xy(from_iso, to_iso, ticker, interval, to_cache=True)
    ...
    mlp_reg = inner_train_mlp_uni_reg(init_mlp_uni_reg(), X, y, mlp_params_distr)
    ...
    log_metrics(cv_metrics, "mlp nested cv test")
    log_metrics(metrics, "mlp full")
    log_metrics(naive_metrics, "naive full")
